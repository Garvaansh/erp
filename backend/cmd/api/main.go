...
func main() {
	...
	requireAuth := middleware.RequireAuth(queries)
	authService := auth.NewAuthService(queries)
	authHandler := handlers.NewAuthHandler(authService)
	itemService := services.NewItemService(queries, dbpool)
	inventoryService := services.NewInventoryService(dbpool, itemService)
	finishedGoodsCommandService := services.NewFinishedGoodsCommandService(dbpool)
	...
	userCommandService := services.NewUserCommandService(dbpool)
	userQueryService := services.NewUserQueryService(dbpool)
	vendorCommandService := services.NewVendorCommandService(dbpool)
	vendorQueryService := services.NewVendorQueryService(dbpool)
	customerCommandService := services.NewCustomerCommandService(dbpool)
	customerQueryService := services.NewCustomerQueryService(dbpool)
	salesOrderCommandService := services.NewSalesOrderCommandService(dbpool)
	orderQueryService := services.NewOrderQueryService(dbpool)
	...
	userHandler := handlers.NewUserHandler(userQueryService, userCommandService, requestValidator)
	vendorHandler := handlers.NewVendorHandler(vendorCommandService, vendorQueryService, requestValidator)
	customerHandler := handlers.NewCustomerHandler(customerCommandService, customerQueryService, requestValidator)
	orderHandler := handlers.NewOrderHandler(salesOrderCommandService, orderQueryService, requestValidator)
	...
	itemsGroup := api.Group("/items", requireAuth)
	...
	itemsGroup.Patch("/:id/threshold", itemHandler.UpdateThreshold)

	inventoryGroup := api.Group("/inventory", requireAuth)
	...
	inventoryGroup.Patch("/batches/:id/status", inventoryHandler.UpdateBatchStatus)
	inventoryGroup.Get("/raw-materials", inventoryHandler.GetRawMaterialMaster)
	inventoryGroup.Get("/raw-materials/:id/summary", inventoryHandler.GetRawMaterialSummary)
	inventoryGroup.Get("/raw-materials/:id/batches", inventoryHandler.GetRawMaterialBatches)
	...
	productionGroup := api.Group("/production", requireAuth)
	...
	ordersGroup := api.Group("/orders", requireAuth)
	ordersGroup.Get("/", orderHandler.ListSalesOrders)
	ordersGroup.Post("/", orderHandler.CreateSalesOrder)
	ordersGroup.Get("/:id", orderHandler.GetSalesOrderDetail)
	ordersGroup.Get("/:id/allocations", orderHandler.GetOrderAllocations)
	ordersGroup.Post("/:id/dispatch", orderHandler.DispatchSalesOrder)
	ordersGroup.Post("/:id/cancel", orderHandler.CancelSalesOrder)

	paymentsGroup := api.Group("/payments", requireAuth)
	paymentsGroup.Post("/", paymentHandler.CreatePayment)
	paymentsGroup.Get("/", paymentHandler.ListPayments)

	financeGroup := api.Group("/finance", requireAuth)
	...
	usersGroup := api.Group("/users", requireAuth, middleware.RequireRole("ADMIN"))
	usersGroup.Get("/", userHandler.ListUsers)
	usersGroup.Post("/", userHandler.CreateUser)
	usersGroup.Get("/:userId", userHandler.GetUserByID)
	usersGroup.Patch("/:userId", userHandler.UpdateUser)
	usersGroup.Post("/:userId/password", userHandler.ChangePassword)

	vendorsGroup := api.Group("/vendors", requireAuth)
	vendorsGroup.Get("/", vendorHandler.ListVendors)
	vendorsGroup.Post("/", vendorHandler.CreateVendor)
	vendorsGroup.Patch("/:id", vendorHandler.UpdateVendor)
	vendorsGroup.Get("/:id/profile", vendorHandler.GetVendorProfile)

	customersGroup := api.Group("/customers", requireAuth)
	customersGroup.Get("/search", customerHandler.SearchCustomers)
	customersGroup.Post("/", customerHandler.CreateCustomer)

	inventoryAdjGroup := api.Group("/inventory", requireAuth)
	...
	inventoryAdjGroup.Get("/finished-goods/:itemId/reservations", orderHandler.GetFinishedGoodReservations)
	inventoryAdjGroup.Get("/batches/:code/reservations", orderHandler.GetBatchReservations)

	reportsGroup := api.Group("/reports", requireAuth)
