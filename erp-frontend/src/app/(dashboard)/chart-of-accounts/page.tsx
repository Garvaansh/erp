...
export default function ChartOfAccountsPage() {
    const [list, setList] = useState<ChartOfAccount[]>([]);
    ...
    const fetchList = async () => {
        try {
            setLoading(true);
            setError(null);
            const res = await api.get<ListResponse>("/finance/chart-of-accounts", { params: { limit: PAGE_SIZE, offset } });
    ...
    const openCreate = () => { setEditing(null); setForm({ code: "", name: "" }); setShowModal(true); };
    const openEdit = (row: ChartOfAccount) => { setEditing(row); setForm({ code: row.code, name: row.name }); setShowModal(true); };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editing) {
                await api.put(`/finance/chart-of-accounts/${editing.id}`, { name: form.name });
            } else {
                await api.post("/finance/chart-of-accounts", { code: form.code.trim(), name: form.name.trim() });
            }
            setShowModal(false);
            fetchList();
        } catch (err: unknown) {
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            await api.delete(`/finance/chart-of-accounts/${deleteTarget.id}`);
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Define account structure for general ledger.</p>
                </div>
                <button onClick={openCreate} className="bg-primary text-primary-foreground hover:bg-primary/90 px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-medium"><Plus className="w-4 h-4" /> Add Chart</button>
            </div>
            ...
            <div className="bg-card/80 border border-border rounded-2xl shadow-xl overflow-hidden">
                {loading && !list.length ? (
                    ...
                    <div className="p-12 text-center text-muted-foreground"><Banknote className="w-12 h-12 mx-auto mb-4 opacity-50" /><p>No chart of accounts yet.</p></div>
                ) : (
                    <>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
                    <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-md p-6">
                        <h2 className="text-lg font-semibold text-foreground mb-4">{editing ? "Edit Chart of Accounts" : "New Chart of Accounts"}</h2>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
                    <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-sm p-6">
                        <p className="text-foreground font-medium">Delete &quot;{deleteTarget.name}&quot;?</p>
