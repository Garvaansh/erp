...
import {
  Building2, UserPlus, Phone, Mail, MapPin, CreditCard,
  Search, X, Check, Ban, Pencil, ChevronDown, Loader2, FileText,
} from "lucide-react";
import { getVendors, createVendor, updateVendor } from "@/features/vendors/api";
import type { Vendor, CreateVendorPayload, UpdateVendorPayload } from "@/features/vendors/types";
...
function CreateVendorSlideout({ open, onClose }: { open: boolean; onClose: () => void }) {
      ...
      <div className="relative w-full max-w-md bg-[var(--sidebar)] border-l border-[var(--erp-border-subtle)] shadow-2xl erp-slide-in overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--erp-border-subtle)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-[var(--erp-text-primary)]">New Vendor</h2>
              <p className="text-[10px] text-[var(--erp-text-muted)]">Register a supplier</p>
            </div>
          </div>
          <button type="button" onClick={onClose} className="p-1.5 text-[var(--erp-text-muted)] hover:text-[var(--erp-text-primary)] transition-colors">
            <X className="size-4" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {[
            { name: "name", label: "Company Name", required: true, placeholder: "e.g. Tata Steel Ltd", icon: Building2 },
            { name: "contact_person", label: "Contact Person", placeholder: "Full name", icon: UserPlus },
            { name: "phone", label: "Phone", placeholder: "+91 ...", icon: Phone },
            { name: "email", label: "Email", placeholder: "vendor@company.com", icon: Mail },
            { name: "gstin", label: "GSTIN", placeholder: "22AAAAA0000A1Z5", icon: FileText },
            { name: "payment_terms", label: "Payment Terms", placeholder: "Net 30", icon: CreditCard },
          ].map(({ name, label, required, placeholder, icon: Icon }) => (
            <div key={name}>
              <label className="block text-xs font-medium text-[var(--erp-text-secondary)] mb-1.5">{label}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--erp-text-secondary)] mb-1.5">Address</label>
          ...
          <button type="submit" disabled={mutation.isPending}
            ...
            {mutation.isPending ? <Loader2 className="size-4 animate-spin" /> : <><Building2 className="size-4" />Register Vendor</>}
          </button>
        </form>
      </div>
    </div>
...
function EditVendorRow({ vendor, onDone }: { vendor: Vendor; onDone: () => void }) {
  const qc = useQueryClient();
  const [error, setError] = useState("");
  const toggle = useMutation({
    mutationFn: (p: UpdateVendorPayload) => updateVendor(vendor.id, p),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["vendors"] }); onDone(); },
    onError: (e) => setError(e instanceof Error ? e.message : "Update failed"),
  });

  return (
    <div className="flex items-center gap-2 mt-2">
      <button type="button" disabled={toggle.isPending} onClick={() => toggle.mutate({ is_active: !vendor.is_active })}
      ...
      <button type="button" onClick={onDone} className="text-[10px] font-semibold px-2.5 py-1 rounded-md bg-[var(--erp-bg-surface)] text-[var(--erp-text-muted)] hover:text-[var(--erp-text-primary)] transition-colors">Cancel</button>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[var(--erp-text-primary)]">Vendor Management</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="erp-card-static p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-[var(--erp-text-primary)] tabular-nums">{vendors.length}</p>
          </div>
        </div>
        <div className="erp-card-static p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-[var(--erp-text-primary)] tabular-nums">{vendors.filter(v => v.is_active).length}</p>
          </div>
        </div>
        <div className="erp-card-static p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-[var(--erp-text-primary)] tabular-nums">{vendors.filter(v => v.gstin).length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="erp-card-static overflow-hidden">
        <div className="flex items-center gap-3 border-b border-[var(--erp-border-subtle)] p-4">
          ...
          <span className="text-xs text-[var(--erp-text-muted)]">{filtered.length} vendor{filtered.length !== 1 ? "s" : ""}</span>
        </div>

        {isLoading ? (
          <div className="flex items-center justify-center py-16"><Loader2 className="size-6 text-[var(--erp-accent)] animate-spin" /></div>
        ) : error ? (
          <div className="flex items-center justify-center py-16 text-sm text-red-400">Failed to load vendors.</div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            <Building2 className="size-10 text-[var(--erp-text-muted)]" />
            <p className="text-sm text-[var(--erp-text-muted)]">{search ? "No vendors match" : "No vendors registered"}</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="erp-table w-full">
              ...
              <tbody>
                {filtered.map((v, i) => (
                  <tr key={v.id} className="erp-table-row erp-fade-in" style={{ animationDelay: `${i * 30}ms` }}>
                    <td className="erp-table-td">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-[var(--erp-text-primary)] truncate">{v.name}</p>
                          {v.email && <p className="text-[10px] text-[var(--erp-text-muted)] truncate">{v.email}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="erp-table-td">
                      <div>
                        {v.contact_person && <p className="text-xs text-[var(--erp-text-primary)]">{v.contact_person}</p>}
                        {v.phone && <p className="text-[10px] text-[var(--erp-text-muted)]">{v.phone}</p>}
                      </div>
                    </td>
                    <td className="erp-table-td">
                      <span className="text-xs text-[var(--erp-text-secondary)] font-mono">{v.gstin || "—"}</span>
                    </td>
                    <td className="erp-table-td">
                      <span className="text-xs text-[var(--erp-text-secondary)]">{v.payment_terms || "—"}</span>
                    </td>
                    <td className="erp-table-td">{statusBadge(v.is_active)}</td>
                    <td className="erp-table-td text-right">
                      {editingId === v.id ? (
                        <EditVendorRow vendor={v} onDone={() => setEditingId(null)} />
                      ) : (
      ...
      <CreateVendorSlideout open={showCreate} onClose={() => setShowCreate(false)} />
    </div>
  );
}
