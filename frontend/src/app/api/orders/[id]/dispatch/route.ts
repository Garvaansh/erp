...
export async function POST(
  ...
  try {
    response = await fetchWithTimeout(
      `${backendURL}/api/v1/orders/${encodeURIComponent(orderId)}/dispatch`,
      {
        method: "POST",
        headers: {
  ...
  if (!response.ok) {
    return apiError(
      readMessage(payload, "Failed to dispatch order"),
      response.status,
      unwrapBackendPayload(payload),
    );
  }

  return apiSuccess("Order dispatched", unwrapBackendPayload(payload));
}
