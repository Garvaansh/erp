...
export async function GET(
  ...
  try {
    response = await fetchWithTimeout(
      `${backendURL}/api/v1/orders/${encodeURIComponent(orderId)}`,
      {
        method: "GET",
        headers: {
  ...
  if (!response.ok) {
    return apiError(
      readMessage(payload, "Failed to load order detail"),
      response.status,
      unwrapBackendPayload(payload),
    );
  }

  return apiSuccess("Order detail loaded", unwrapBackendPayload(payload));
}
