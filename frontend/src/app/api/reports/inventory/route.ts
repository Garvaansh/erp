...
export async function GET(request: Request) {
  ...
  let res: Response;
  try {
    res = await fetchWithTimeout(`${backendURL}/api/v1/reports/inventory?days=${days}`, {
      method: "GET",
  ...
  if (!res.ok) return apiError(readMessage(payload, "Failed to load inventory report"), res.status);
