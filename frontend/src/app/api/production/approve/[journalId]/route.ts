...
export async function PATCH(
  ...
  try {
    backendResponse = await fetchWithTimeout(
      `${backendURL}/api/v1/production/approve/${encodeURIComponent(journalId)}`,
      {
        method: "PATCH",
        headers: {
  ...
  if (!backendResponse.ok) {
    return apiError(
      readMessage(payload, "Failed to approve journal"),
  ...
  if (!result) {
    return apiError("Malformed approval response", 502);
  }

  return apiSuccess("Journal approved", result, 200);
}
